// Standard Quran Mushaf (Hafs 'an 'Asim, 604 pages)
// Maps the FIRST page of each Surah
export const SURAH_FIRST_PAGE: Record<number, number> = {
  ...
  81: 586, 82: 587, 83: 587, 84: 588, 85: 590, 86: 591, 87: 591, 88: 592,
...
// Build reverse map: page → surah ID that starts ON or BEFORE this page
const sortedSurahs = Object.entries(SURAH_FIRST_PAGE)
  .map(([id, page]) => ({ id: Number(id), page }))
  .sort((a, b) => a.page - b.page);

export function getSurahForPage(pageNum: number): number {
  let surahId = 1;
  for (const entry of sortedSurahs) {
    if (entry.page <= pageNum) surahId = entry.id;
    else break;
  }
  return surahId;
}

// Generate full page list: 604 pages
export interface PageEntry {
  page: number;
  surahId: number;
}

export const PAGE_LIST: PageEntry[] = Array.from({ length: 604 }, (_, i) => ({
  page: i + 1,
  surahId: getSurahForPage(i + 1),
}));
